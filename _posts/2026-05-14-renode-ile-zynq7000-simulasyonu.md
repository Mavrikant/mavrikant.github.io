...
- **C# çekirdek, Python ile genişletilebilirlik:** Yeni bir çevre birimini birkaç düzine satırda eklemek mümkündür
...
Burada `Cadence_UART`, `XilinxGPIOPS` gibi tip isimleri, Renode'un içinde C# olarak modellenmiş çevre birimi sınıflarına karşılık gelir. Adres alanı (`0xE0001000`), reset davranışı ve IRQ numarası, fiziksel Zynq7000 ile birebir eşleşir. Yani aynı sürücü kodu, hem Renode üzerinde hem gerçek kart üzerinde değişiklik gerektirmeden çalışır.
...
Bir çevre birim eksikse, Python ile bir saatte basit bir model yazılabilir. Örneğin, basit bir 32-bit "sensör değeri" register'ı sunan custom peripheral:

```python
# my_sensor.py
from Antmicro.Renode.Peripherals.Bus import IDoubleWordPeripheral

class MySensor(IDoubleWordPeripheral):
    def __init__(self):
        self.value = 0
    ...
    def WriteDoubleWord(self, offset, val):
        if offset == 0x04:
            self.value = val
...
Renode konsolundan `sysbus.mySensor value 42` yazarak simülasyon sırasında sensör değerini değiştirebilir; firmware'iniz tam o anda farklı bir koda dallanacaktır. Bu, **fault injection** testleri için son derece güçlü bir araçtır.
...
FPGA'da yapılacak işin **davranışsal modelini** Python ya da C# olarak yazarsınız. AXI register haritası taklit edilir, içerideki algoritma host kodunda çalışır. RTL test edilmez ama yazılım entegrasyonu sorunsuz ilerler. Yukarıdaki `MySensor` örneği aslında bu yaklaşımın tipik bir örneğidir.
...
İkisinin arası bir yol: HDL yazmadan, AXI protokolünün doğru ritmini taklit eden bir Python/C# peripheral yazarsınız. AXI handshake'i, burst transfer davranışı ve back-pressure gibi konuları modellersiniz. Sürücü geliştirme ve yazılım entegrasyonu için yeterli; RTL henüz olgunlaşmamış olsa bile çalışır.
